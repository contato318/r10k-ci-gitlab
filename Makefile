...
exec_bash_env: ## Run bash (container) simulate .env
	docker run -it --rm -v $(CURRENT_DIR):/app -v $(CURRENT_DIR)/Puppetfile:/Puppetfile -v $(CURRENT_DIR)/CACHE/:/var/cache/r10k --env-file $(CURRENT_DIR)/.env $(CONTEXTO) bash
...
send_git: ## Send to git (build, tests and send)
	@git pull ||   echo -e "$(ORANGE_COLOR)Could not pull$(NO_COLOR)\n"
	@make build
...
test-docker:  ## Docker tests
	...
	@sleep 1; docker run --name teste-list-docker-$(CONTEXTO) --rm -it  $(CONTEXTO) r10k version | grep 10k && \
